...
/**
 * Label kelas untuk kalimat "... pada kelas X".
 * Sebagian label sudah memuat kata "Kelas" (mis. "Kelas 1"), jadi awalan itu
 * dibuang agar tidak menjadi "pada kelas Kelas 1".
 */
export function labelKelasInline(label) {
  const teks = (label ?? '').trim();
  if (!teks) return GARIS_ISIAN;
  return teks.replace(/^kelas\s+/i, '');
}
...
/**
 * Cap (stempel) lembaga. Gambarnya menumpang tabel tanda tangan dengan
 * kunci sintetis `cap:<lembaga>`, sama seperti tanda tangan pemimpin arsip,
 * sehingga tidak perlu tabel gambar baru.
 */
export function capTtdKey(lembaga) {
  return `cap:${lembaga}`;
}

/**
 * Kalibrasi cap bawaan. Sama seperti tanda tangan, ditambah `rot` — sudut
 * putar dalam derajat, supaya cap bisa dimiringkan seperti stempel asli.
 */
export const CAP_DEFAULT = { x: 0, y: 0, scale: 100, rot: 0 };

/** Lengkapi kalibrasi cap tiap lembaga agar bentuknya selalu utuh. */
