...
/**
 * Benar bila label ini sudah dipakai tahun ajaran LAIN (biasanya arsip).
 * Diperiksa di route supaya pesannya sampai ke pemakai apa adanya.
 */
export async function labelTaDipakaiLain(label) {
  const sql = await getSql();
  if (!sql) return false;
  const aktif = await activeTaId();
  const rows = await sql`
    SELECT 1 FROM tahun_ajaran
     WHERE lower(label) = lower(${String(label).trim()}) AND id <> ${aktif ?? ""}
     LIMIT 1
  `;
...
/**
 * Ganti label tahun ajaran yang sedang aktif.
 *
 * Hanya labelnya yang berubah; `id` sengaja dibiarkan apa adanya karena ia
 * dirujuk oleh kelas, santri, ujian, nilai, dan seluruh arsip lewat foreign
 * key. Mengubah id berarti memindahkan semua baris itu, dan id memang tidak
 * pernah tampil ke pemakai — yang dibaca orang hanyalah labelnya.
 *
 * Dipakai untuk membetulkan label T.A. berjalan, bukan untuk berpindah
 * tahun ajaran (itu tugas `arsipkanTa`).
 */
export async function ubahLabelTa(newLabel) {
  const sql = await getSql();
  if (!sql) return null;

  const label = String(newLabel ?? "").trim();
  if (!label) throw new Error("Label tahun ajaran wajib diisi.");

  const aktif = await activeTaId();
  if (!aktif) throw new Error("Tidak ada tahun ajaran aktif.");

  await sql`UPDATE tahun_ajaran SET label = ${label} WHERE id = ${aktif}`;
  return { id: aktif, label };
}
