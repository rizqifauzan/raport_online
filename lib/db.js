...
export async function assembleState() {
  ...
  const [ta, kelas, santri, ujian, nilai, karakter, kenaikan, lock, pimpinan, gurus, users, nilaiMapel, mapel, caps] =
    await Promise.all([
  ...
  return {
    data: {
      ...
      // Kalibrasi cap lembaga: { TPQ: {x,y,scale}, Madin: {...} }
      cap: Object.fromEntries(
        caps.map((c) => [
          c.lembaga,
          { x: c.cap_x, y: c.cap_y, scale: c.cap_scale, rot: c.cap_rot ?? 0 },
        ]),
      ),
    },
    taId: taAktif.id,
...
/**
 * Simpan kalibrasi cap satu lembaga. Gambarnya sendiri lewat jalur tanda
 * tangan (`guru_ttd`, kunci 'cap:<lembaga>'), sama seperti TTD pemimpin.
 */
export async function setCapTtd(lembaga, ttd) {
  const sql = await getSql();
  if (!sql) return;
  await sql`
    INSERT INTO lembaga_cap (lembaga, cap_x, cap_y, cap_scale, cap_rot)
    VALUES (${lembaga}, ${Number(ttd?.x) || 0}, ${Number(ttd?.y) || 0},
            ${Number(ttd?.scale) || 100}, ${Number(ttd?.rot) || 0})
    ON CONFLICT (lembaga) DO UPDATE
      SET cap_x = EXCLUDED.cap_x,
          cap_y = EXCLUDED.cap_y,
          cap_scale = EXCLUDED.cap_scale,
          cap_rot = EXCLUDED.cap_rot
  `;
}
